...
function ProfilePic({ handleChangePic }) {
  ...
  //its reference the when you click on upload pic its trigger the input 
  ...
  let token = sessionStorage.getItem('token')
    ...
    const saveProfilePic = async ()=>{
       try {
          let res = await axios.put(`${url}/uploadProfilePic`,{
            email,imageUrl
          },{
            headers: {
    ...
    <div className='profilePic darkBg '>
      <div className='changePic centered'>
        ...
        <div style={{ borderTop: "1px solid #00000030" }}>
          <button className='upload-btn' style={{ color: "#1EA1F7" }} onClick={()=>handleUploadPic()}>Upload Photo</button>

...
const CreatePost = () => {
    ...
    let previewImage = "https://cdn4.iconfinder.com/data/icons/ionicons/512/icon-image-512.png"
    ...
    useEffect(() => {
        try {
            if (imageUrl) {
                ...
                let res = axios.post(`${url}/users/createPost`, {
                    text: text,
                    image: imageUrl,
                    name: user.fullName,
                    email: email
    ...
    const handlePost = async () => {
        try {
            ...
            data.append("cloud_name", "ajeethcloud")

            //post image to cloudinary create image link
            await fetch("https://api.cloudinary.com/v1_1/ajeethcloud/image/upload", {
                method: "post",
                body: data
            }).then(res => res.json())
            ...
            <div className='d-flex justify-content-center'>
                <div className='create-post'>
                    <div className='post-header d-flex justify-content-around pt-2'>
                        ...
                        <Button variant="primary" style={{ width: '5em', height: '2.5em' }} onClick={() => handlePost()}>Share</Button>
                    </div>
                    ...
                    <div className='card-details'>
                        <div className='card-header'>
                            ...
                            <h6><b>{user.fullName}</b></h6>
                        </div>
                        <div>
                            <textarea type="text" placeholder='write a caption.....' onChange={(e) => setText(e.target.value)}>
                            </textarea>
                        </div>
                    </div>
                </div>
